...
class BookSerializer(serializers.ModelSerializer):
    """Full read-only serializer — returns seller info nested."""
    ...
    def get_cover(self, obj):
        """Return cover_image URL if uploaded, else fall back to cover_url."""
...
class BookCreateSerializer(serializers.ModelSerializer):
    """Write serializer used when authenticated users list a book."""
    class Meta:
        model = Book
        fields = (
            'id', 'title', 'author', 'isbn', 'gtu_code',
    ...
    def create(self, validated_data):
        # Inject authenticated user as seller
